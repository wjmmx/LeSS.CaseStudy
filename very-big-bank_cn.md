...
遗憾的是，手动测试在 VBB 中早已存在很久，这意味着新组建的团队中也有手动测试人员。为了对此进行补救，整个团队开始大力推动测试自动化和优秀工程实践的分享。其中一位教练（Stuart）通过培训和辅导对此提供了支持。主要的承载工具是工程社区（参见LeSS指南：*社区*) ，并在出现特定需求时提供有针对性的培训。鼓励团队从传统的"测试待发布版本"流程过渡到作为"完成的定义"的一部分的特性测试，可以是手动或自动。
